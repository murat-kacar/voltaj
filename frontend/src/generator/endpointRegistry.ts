...
export const ENDPOINT_CATEGORIES: EndpointCategory[] = [
  ...
  { id: 'projects', label: 'Projeler & Hakediş' },
...
export const ALL_ENDPOINTS: EndpointDefinition[] = [
  // ==========================================================================
  // 1. WORK ORDERS (İş Emirleri)
  // ==========================================================================
  {
    id: 'wo-list',
    method: 'GET',
    path: '/api/workorders',
    title: 'İş Emirlerini Listele',
    category: 'workorders',
    description: 'Sistemdeki iş emirlerini sayfalayarak listeler.',
    queryParams: [
  ...
  },
  {
    ...
    category: 'workorders',
    description: 'Müşteri için yeni bir saha servis / taahhüt iş emri oluşturur.',
    bodyFields: [
  ...
  },
  {
    ...
    title: 'İş Emri Detayını Getir',
    category: 'workorders',
    description: 'Belirtilen ID değerine sahip iş emrini getirir.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    generateMock: () => ({
  ...
  },
  {
    id: 'wo-assign',
    method: 'POST',
    path: '/api/workorders/{id}/assign',
    title: 'İş Emrini Teknisyene Ata',
    category: 'workorders',
    description: 'İş emrini sorumlu teknik personele atar.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    bodyFields: [
      { name: 'employeeUserId', label: 'Atanacak Teknisyen (User ID)', type: 'fk', fkEntity: 'User', required: true },
    ],
    generateMock: (l) => ({
      path: { id: '313e4fc8-9f33-4f96-be0d-b45cb0ca1ff3' },
      body: { employeeUserId: l?.userId || '2aba7edf-5c17-4625-a1d6-e77cdf1353e0' },
    }),
  },
  {
    id: 'wo-start',
    method: 'POST',
    path: '/api/workorders/{id}/start',
    title: 'İş Emrini Başlat (Start)',
    category: 'workorders',
    description: 'İş emrinin durumunu InProgress yapar ve hedef tamamlanma tarihi belirler.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    bodyFields: [
      { name: 'targetCompletionDate', label: 'Hedef Bitiş Tarihi', type: 'date', required: false },
    ],
    generateMock: () => ({
      path: { id: '313e4fc8-9f33-4f96-be0d-b45cb0ca1ff3' },
      body: { targetCompletionDate: '2026-10-15' },
    }),
  },
  {
    id: 'wo-checkin',
    method: 'POST',
    path: '/api/workorders/{id}/check-in',
    title: 'Sahaya Giriş Yap (Check-In)',
    category: 'workorders',
    description: 'Teknisyenin sahaya varışını zaman kaydı olarak işler.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    bodyFields: [
      { name: 'notes', label: 'Giriş Notu', type: 'text', helperText: 'Örn: Sahaya ulaşıldı, güvenlik brifingi alındı.' },
    ],
    generateMock: () => ({
      path: { id: '313e4fc8-9f33-4f96-be0d-b45cb0ca1ff3' },
      body: { notes: 'Saha giriş kontrolü tamamlandı, panolarda enerji kesildi.' },
    }),
  },
  {
    id: 'wo-hold',
    method: 'POST',
    path: '/api/workorders/{id}/hold',
    title: 'İş Emrini Beklemeye Al (Hold)',
    category: 'workorders',
    description: 'İş emrini parça veya müşteri onayı gerekçesiyle beklemeye alır.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    bodyFields: [
      { name: 'reason', label: 'Bekleme Gerekçesi', type: 'text', required: true },
    ],
    generateMock: () => ({
      path: { id: '313e4fc8-9f33-4f96-be0d-b45cb0ca1ff3' },
      body: { reason: 'Yeni yedek parça siparişi bekleniyor (Tedarik süresi 2 gün)' },
    }),
  },
  {
    id: 'wo-resume',
    method: 'POST',
    path: '/api/workorders/{id}/resume',
    title: 'İş Emrini Devam Ettir (Resume)',
    category: 'workorders',
    description: 'Beklemedeki iş emrini tekrar aktif çalışma durumuna geçirir.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    generateMock: () => ({
  ...
  },
  {
    id: 'wo-complete',
    method: 'POST',
    path: '/api/workorders/{id}/complete',
    title: 'İş Emrini Tamamla (Complete)',
    category: 'workorders',
    description: 'Müşteri imzası ve servis fotoğrafı ile iş emrini başarıyla kapatır.',
    pathParams: [
      { name: 'id', label: 'İş Emri ID', type: 'fk', fkEntity: 'WorkOrder', required: true },
    ],
    bodyFields: [
      { name: 'signatureData', label: 'Müşteri İmza Verisi (Base64/Metin)', type: 'text', required: false },
      { name: 'proofOfWorkPhotoUrl', label: 'Servis Fotoğraf URL', type: 'text', required: false },
    ],
    generateMock: () => ({
      path: { id: '313e4fc8-9f33-4f96-be0d-b45cb0ca1ff3' },
      body: {
        signatureData: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...',
        proofOfWorkPhotoUrl: 'https://storage.voltflow.dev/proofs/wo_completion_test.jpg',
      },
    }),
  },
  ...
  // ==========================================================================
  {
    ...
    method: 'GET',
    path: '/api/quotes',
    title: 'Teklifleri Listele',
    category: 'quotes',
    description: 'Sistemdeki satış ve taahhüt tekliflerini listeler.',
    queryParams: [
      { name: 'search', label: 'Arama Terimi', type: 'text' },
      {
        name: 'state',
        label: 'Teklif Durumu',
        type: 'enum',
        options: [
          { value: '', label: 'Tümü' },
          { value: 'Draft', label: 'Taslak (Draft)', color: 'default' },
          { value: 'Issued', label: 'Gönderildi (Issued)', color: 'info' },
          { value: 'Accepted', label: 'Kabul Edildi (Accepted)', color: 'success' },
          { value: 'Rejected', label: 'Reddedildi (Rejected)', color: 'error' },
        ],
      },
  ...
  },
  {
    ...
    category: 'quotes',
    description: 'Müşteriye özel kalemleri ve KDV hesaplamaları olan yeni bir teklif oluşturur.',
    bodyFields: [
      { name: 'customerId', label: 'Müşteri', type: 'fk', fkEntity: 'Customer', required: true },
      { name: 'title', label: 'Teklif Konusu / Başlığı', type: 'text', required: true },
      { name: 'validUntil', label: 'Geçerlilik Tarihi', type: 'date', required: false },
      { name: 'notes', label: 'Teklif Şartları ve Notlar', type: 'text', required: false },
    ],
    generateMock: (l) => ({
      body: {
        customerId: l?.customerId || '572a1548-dbdc-4b71-9c60-a2ea03d6d03f',
        title: `Endüstriyel Pano ve Otomasyon Revizyonu (${rand(100, 999)})`,
        validUntil: '2026-11-30',
        notes: 'Teklif 30 gün geçerlidir. Montaj ve devreye alma dahildir.',
        lines: [
          { description: 'Schneider 630A Kompakt Şalter', quantity: 2, unitPrice: 18500, unit: 'Adet', kind: 'Material', vatRate: 20 },
          { description: 'Saha Montaj ve Kablolama İşçiliği', quantity: 1, unitPrice: 25000, unit: 'Paket', kind: 'Labor', vatRate: 20 },
        ],
      },
    }),
  },
  {
    id: 'quote-issue',
    method: 'POST',
    path: '/api/quotes/{id}/issue',
    title: 'Teklifi Müşteriye Gönder (Issue)',
    category: 'quotes',
    description: 'Taslak teklifi resmileştirerek Issued durumuna geçirir.',
    pathParams: [
      { name: 'id', label: 'Teklif ID', type: 'fk', fkEntity: 'Quote', required: true },
    ],
    generateMock: () => ({
      path: { id: '3fa85f64-5717-4562-b3fc-2c963f66afa6' },
    }),
  },
  {
    id: 'quote-accept',
    ...
    title: 'Teklifi Kabul Et (Accept)',
    category: 'quotes',
    description: 'Müşterinin onayladığı teklifi Accepted durumuna alır ve kapora yüzdesi tanımlar.',
    pathParams: [
      { name: 'id', label: 'Teklif ID', type: 'fk', fkEntity: 'Quote', required: true },
    ],
    bodyFields: [
      { name: 'requiredDepositPercentage', label: 'Zorunlu Ön Ödeme / Kapora (%)', type: 'number', required: true },
    ],
    generateMock: () => ({
      path: { id: '3fa85f64-5717-4562-b3fc-2c963f66afa6' },
      body: { requiredDepositPercentage: 25 },
    }),
  },
  {
    id: 'quote-to-wo',
    method: 'POST',
    path: '/api/quotes/{id}/work-order',
    title: 'Teklifi İş Emrine Dönüştür',
    category: 'quotes',
    description: 'Kabul edilen teklif kalemlerini otomatik olarak yeni bir iş emrine dönüştürür.',
    pathParams: [
      { name: 'id', label: 'Teklif ID', type: 'fk', fkEntity: 'Quote', required: true },
    ],
    generateMock: () => ({
  ...
  // ==========================================================================
  // 7. PROJECTS & BILLING (Projeler & Hakediş)
  // ==========================================================================
  {
    id: 'proj-list',
    method: 'GET',
    path: '/api/projects',
    title: 'Taahhüt Projelerini Listele',
    category: 'projects',
    description: 'Elektrik taahhüt ve tesis projelerini listeler.',
    queryParams: [
      { name: 'limit', label: 'Limit', type: 'number' },
    ],
    generateMock: () => ({
  ...
  },
  {
    ...
    method: 'POST',
    path: '/api/projects',
    title: 'Yeni Taahhüt Projesi Aç',
    category: 'projects',
    description: 'Müşteri için bütçeli ana taahhüt projesi tanımlar.',
    bodyFields: [
      { name: 'customerId', label: 'Müşteri', type: 'fk', fkEntity: 'Customer', required: true },
      { name: 'name', label: 'Proje Adı', type: 'text', required: true },
      { name: 'budget', label: 'Sözleşme Bütçesi (₺)', type: 'number', required: true },
    ],
    generateMock: (l) => ({
      body: {
        customerId: l?.customerId || '572a1548-dbdc-4b71-9c60-a2ea03d6d03f',
        name: `Organize Sanayi Güneş Enerjisi Santrali Kurulumu (${rand(10, 99)})`,
        budget: rand(500, 2500) * 1000,
      },
    }),
  },
