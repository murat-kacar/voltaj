...
| **CI** | New `.github/workflows/security.yml` (a new file; `ci.yml` is untouched and `deploy.yml` only forwards the optional seed secrets of ADR 0009): on push, pull request, weekly and on demand; `permissions: contents: read`; every action pinned to a commit SHA. Job `supply-chain` runs `make check-secrets audit license-policy` and uploads the SBOMs. Job `asvs-audit` asks two questions: on every push and pull request `make check-asvs-integrity` (is the audit record sound - must pass); weekly and on demand `make check-asvs` (is it complete - red until every in-scope requirement is assessed, deliberately not `continue-on-error`). The first run on GitHub showed `supply-chain` green on Ubuntu and `asvs-audit` red as designed, which is why the push/PR run was split from the weekly one. |
